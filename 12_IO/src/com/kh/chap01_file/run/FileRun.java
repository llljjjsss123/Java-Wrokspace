...
public class FileRun {
	...
	public static void main(String[] args) {
		...
		//4. 별도의 경로 지정 없이 폴더 생성 후 파일 생성하기->프로젝트 내에 생성될것.
		...
		/*
		 * 프로그램 상의 데이터를 외부매체(모니터, 스피커, 자바프로그램, 파일 등등)로 출력하거나 입력장치(키보드, 마우스)로
		 * 입력받는 과정을 진해하고자 한다면 반드시 외부매체와의 통로를 만들어야한다.->통로==스트링
		 * 
		 * 스트링의 특징:
		 * -단방향: 입력이면 입력, 출력이면 출력
		 * 			입력, 출력 스트림이 따로 존내
		 * 			즉 , 동시에 입출력하고자 한다면 하나의 스트림으로 불가능함
		 * 
		 ...
		 * 스트림의 구분
		 * -통로사이즈:
		 * 바이트스트림: 1Byte짜리가 이동할 수 있을정도의 사이즈(좁은통로)->입력(XXXInputStream)/출력(XXXOutputStream)
		 * 문자스트림: 2Byte짜리가 이동 할수 있는 사이즈(넓은 통로)=>입력(XXXReader)/출력(xxxWriter)
		 * 
		 * -외부매테와의 직접적인 연결여부:
		 * 	기반스트림: 외부매체와 직접적으로 연결되는 통로
		 * 	보조스트림: 기반스트림만으로는 부족한 성능을 향상시켜주는 용도의 스트림(직접적으로 외부매테와 연결할수 없음, 즉 단독사용이 불가)
		 * (보조해주는 기능: ㅅ속도향상, 자료형 변환, 객체단위의 입출력 보조등등)
		 * 단, 보조스트입은 단독적으로 사용이 불가능하므로 반드시 외부매체와 직접적으로 연걸 가능한 기반스트림과 함께사용한다.
		 * 
		 */
}
}