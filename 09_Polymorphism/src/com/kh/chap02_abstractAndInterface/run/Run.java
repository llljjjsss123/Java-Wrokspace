...
public class Run {
	...
	public static void main(String[] args) {
		...
		//Sports s=new Sports();
		//추상클래스로 절대 생성 불가 =>미완성 클래스 이기 때문
		
		Sports s;//단, 객체 생성만 안될뿐 레퍼런스 변수로는 사용가능.
		s=new Football();//즉 다형성을 적용해서 자식객체를 받아주는 용도로 사용사능
		 ...
		 * 추상 메소드가 존재하는 순간 반드시 추상클래스로 정의
		 * (일반필드+일반메도스 +(추상메소드(생략가능))
		 * 
		 * 단, 추상메소드가 굳이 없어도 추상클래스로 둘수 있긴함
		 * 언제?=>클래스가 아직 구체적으로 덜 구현됨 상태인것으로 보이는 경우(개념적)
