...
//		p2.sleep();//아기: 건강도 +3>6.5 64
		...
		System.out.println(b2);
		/*
		 * 클래스에서 클래스 상속받을때:extends클래스(단일상속만 가능):화살표 실선
		 * 클래스에서 인터페이스를 구현할때: implements인터페이스,인터페이스(다중구현):화살표 점선
		 ...
		 * 표현법/개수|명시적 abstract 기술해야함  			|묵시적으로 abstracst기술(생략갸능)
		 * 일반메소드여부| 0								|x
		 * 필드		|  일반필드 가질수 있음 					| 상수필드만 가질수 있음
		 * 
		 * 
		 * 배운순서
		 * extends 일반클래스-> extends 추상클래스-->implements인터페이스
		 * 뒤도 갈수록 강제성이 더 짙어짐
		 */
	}
}
